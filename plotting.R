...
plot_model_figure <- function(penguins_clean) {
  ggplot(penguins_clean, aes(x = body_mass_g , y = culmen_length_mm, colour = species)) +
    geom_point() +
...
save_model_plot_png <- function(penguins_clean, filename, size, res, scaling){
  agg_png(filename, width = size, 
          height = size, 
          units = "cm", 
          res = res, 
